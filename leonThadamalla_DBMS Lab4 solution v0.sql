...
-- to prevent two pricing by the same supplier for a given product.
alter table supplier_pricing add constraint unique key (PRO_ID,SUPP_ID);
...
-- rating and Type_of_Service. For Type_of_Service, If rating =5, 
...
select s.supp_id, s.supp_name, avg(rat_ratstars),
	case when avg(rat_ratstars)=5 then 'Excellent Serivce'
		 when avg(rat_ratstars)>4 then 'Good Serivce'
         when avg(rat_ratstars)>2 then 'Average Serivce'
         else 'Poor Serivce'
